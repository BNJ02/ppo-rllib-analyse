...
| 0 | [Résumé du papier PPO](docs/00-resume-papier-ppo.md) | une page : la chaîne $`\hat g`$ → $`L^{CPI}`$ → contrainte TRPO → $`L^{CLIP}`$, et ce que TRPO coûtait |
...
| 6 | [De TRPO à PPO](docs/06-de-trpo-a-ppo.md) | la version longue du n°0 : figures 1 et 2 du papier commentées, clipping cas par cas, tableau comparatif des trois algorithmes, pourquoi « Algorithm**s** » au pluriel, garanties perdues une par une |
